...
describe(Definition) do
  ...
  describe('#definition') do
    it('returns the definition of a term') do
      ...
      expect(test_definition.definition()).to(eq('a round, red fruit from an apple tree'))
    end
  end
  ...
  describe('.find') do
    it('finds a definition based on an id number') do
      test_definition = Definition.new({:definition => 'a round, red fruit from an apple tree', :id => 1})
      test_definition.add
      test_definition2 = Definition.new({:definition => 'a round, red fruit from an apple tree', :id => 1})
